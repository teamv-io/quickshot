  ...
  return (
    <div className="shrink-0 border-t border-white/10 bg-black/30 p-2">
      <div className="flex gap-2 overflow-x-auto">
        {items.length === 0 && (
          <div className="px-2 py-6 text-xs text-zinc-600">No matching captures</div>
        )}
        {items.map((it) => (
          <div
            key={it.id}
            ...
            onDragStart={(e) => {
              e.preventDefault()
              window.api.libraryStartDrag(it.id)
            }}
            title={it.title || new Date(it.created_at).toLocaleString()}
          ...
          >
            ...
            {it.title && (
              <span className="absolute inset-x-0 bottom-0 truncate bg-black/70 px-1.5 py-0.5 text-[10px] text-zinc-200">
                {it.title}
              </span>
            )}
            <div className="absolute right-1 top-1 hidden gap-1 group-hover:flex">
              <button
                onClick={(e) => {
                  e.stopPropagation()
                  window.api.libraryReveal(it.id)
                }}
                title="Reveal in Finder"
                className="flex h-5 w-5 items-center justify-center rounded-full bg-black/70 text-white hover:bg-sky-500"
              >
                <FolderOpen size={11} />
              </button>
              <button
                ...
                title="Delete"
                className="flex h-5 w-5 items-center justify-center rounded-full bg-black/70 text-white hover:bg-red-500"
              >
                <X size={12} />
              </button>
            </div>
